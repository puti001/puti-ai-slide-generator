...
4. **極簡與清爽排版原則 (Clean & Minimalist Guidelines)**：
   為了維持影片的質感，避免畫面被雜亂元素干擾或遮擋背景圖片，你必須遵循以下視覺規範：
   * **不遮擋背景主體**：不使用置中的巨型字卡或大字報，以免破壞精心製作的簡報圖片。
   * **角落小標題**：如需呈現頁面小標題，統一在左上角或角落設定小字體（如 `x: 18, y: 10, size: 36`），且進入動畫以 `pop-in` 或 `fade-in` 為主，不隨意加入會導致文字抖動的漂浮（`float`）或持續強調效果。
   * **依靠卡點字幕傳遞對白**：主要台詞訊息完全依靠底部自動卡點的字幕呈現，保持畫面中央的純淨與連貫。
