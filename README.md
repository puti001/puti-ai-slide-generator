...
每個簡報可以在最開頭（第一張投影片前）宣告全域設定（例如角色聲音與背景音樂），格式如下：
...
3. **動態設計對談主角與編劇**：根據投影片的主題，**動態設計兩個最貼切的主談人/主角名稱**（例如：商業簡報使用「張經理/李秘書」、科技簡報使用「阿哲/小美」、佛學簡報使用「小螺/菩菩」）。將文字大綱改寫為生動的對話，並以 `角色名字：「台詞內容」` 格式交替呈現。`puti_generator.py` 會自動過濾名字，並根據名字中的性別關鍵字（如「女/美/姐」或「男/哲/哥/佛」）智能匹配最適合的男女聲配音。
4. **活用大字報、Morph 與多物件排列效果 (Advanced Visual Guidelines)**：
   不要只會寫平淡的對白與字幕，你必須**主動且活潑地在腳本中加入動態視覺元素**：
   * **巨型大字報**：當台詞包含情緒高昂的關鍵詞（如「火燒洛陽」、「危機」），請在該頁加入字體極大（`size: 72~80`）且顯眼（如紅色 `"#ff3333"`）的 `[object]`，並設定 `entrance: pop-in` 與 `motion: pulse` 作為視覺焦點。
   * **Morph 標題收納**：讓前一頁居中的巨型標題（如 `x: 50, y: 45, size: 72`），在後一頁平滑變形成左上角的小常駐標題（如 `x: 18, y: 10, size: 32`）並加上 `motion: float`，營造電影感轉場。
   * **多物件並列**：提到並列項目時（如「皇宮、太廟、國庫」），宣告多個 `[object]` 並精確計算坐標（如 `x: 30`, `50`, `70`），分別搭配 `fly-in-left`、`pop-in` 等動畫依次呈現。
5. **寫入 script.txt**：將包含上述視覺效果宣告與台詞的內容寫入該資料夾下的 `script.txt`。
